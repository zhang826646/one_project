...
class CirclePost(BaseModel):
    ...
    type = Column(SmallInteger, default=0, comment='类型[1:md|2:html]')
...
class CircleCatalog(BaseModel):
    ...
    id = Column(Integer, primary_key=True)
    catalog_name = Column(String(50), comment='板块名称')
    content = Column(Text(), nullable=True, default='', comment='板块说明')
    deleted = Column(SmallInteger, default=0, comment='删除[0:正常|1:删除]')
